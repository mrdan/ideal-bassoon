...
public class Point implements Comparable<Point> {
    ...
    public double slopeTo(Point that) {
        ...
        // If points are equal, return Double.NEGATIVE_INFINITY
        if (this.x == that.x && this.y == that.y)
            return Double.NEGATIVE_INFINITY;

        double resultX = (that.x - x);
        double resultY = (that.y - y);

        // if Y's are equal, then it's horizontal
        if (resultY == 0)
            return +0.0;

        // if X's are equal, then it's vertical
        if (resultX == 0)
    ...
    public static void main(String[] args) {
        ...
        Point p = new Point(166, 137);
        Point q = new Point(385, 245);
